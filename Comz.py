# -*- coding: UTF-8 -*-.
# Author Tuyul Google
# My Team : Black Coders
# Visit : https://tuyul-google.blogspot.com
...
{ 06 } Compile Base16
{ 07 } Compile Binary
{ 08 } Compile Marshall&Base64
{ 09 } Random Compile { No 1 - 8 }
{ 10 } Encode Random {M,Z,B}
{ 11 } Obfuscate Tuyul Google 1
{ 12 } Obfuscate Tuyul Google 2
{ 13 } Obfuscate Tuyul Google 3
{ 14 } Obfuscate Tuyul Google 4
{ 15 } Encode rot_13
{ 16 } Encode cp424
{ 17 } Encode bz2_codec
{ 18 } Encode utf_32_le
...
{ 24 } Encode utf_16_be
{ 25 } Encode uu_codec
{ 26 } Random Encode [ 15 - 25 ]
{ 27 } Obfuscate Tuyul Google 5
{ 28 } Pyc Inject Not
──────────────────────────────────
{ E } Exit
{ A } About
	...
	def CHOSEE(self):
		...
		if self.CHOSE == "1" or self.CHOSE == "01":
			...
			if _box.flocbox(self.FIL,"cek").ceks() == "ok":
				if _box.flocbox(self.FIL,"syntax").cekSysntax() == "ok":
					ONS["File"] = self.FIL.replace(".py","_enc.py") if ".py" in self.FIL else self.FIL+".py"
					special_Compile_By_Tuyul-Google_gans(self.FIL).somethink_puki()
					_dat.fucking.pantad(SHEL[0],ONS["File"])
				else:
					print >>_sys.stdout,"<-- Invalid Syntax---> "
			else:
			...
			if _box.flocbox(self.FIL,"cek").ceks() == "ok":
				if _box.flocbox(self.FIL,"syntax").cekSysntax() == "ok":
					ONS["File"] = self.FIL.replace(".py","_enc.py") if ".py" in self.FIL else self.FIL+".py"
					special_Compile_By_Tuyul-Google_gans(self.FIL).anymore_flask()
					_dat.fucking.pantad(SHEL[0],ONS["File"])
				else:
					print >>_sys.stdout,"<-- Invalid Syntax---> "
			else:
		...
		if self.INfile:
			...
			if _osz.path.isfile(self.INfile):
				if self.comp == 1:
					...
					if self.OUTFil:
						special_Compile_By_Tuyul-Google_gans(self.INfile).somethink_puki()
						print "Saving As - %s "%(self.OUTFil)
						_dat.fucking.pantad(SHEL[0],self.OUTFil)
					else:
					...
					if self.OUTFil:
						special_Compile_By_Tuyul-Google_gans(self.INfile).anymore_flask()
						print "Saving As - %s "%(self.OUTFil)
						_dat.fucking.pantad(SHEL[0],self.OUTFil)
					else:
